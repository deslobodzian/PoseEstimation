...
public:
	...
	//defining arguments/parameters for CameraGL()
	CameraGL(
		sl::Translation position, 
		sl::Translation direction, 
		sl::Translation vectical = sl::Translation(0,1,0)
	);
	~CameraGL(); // Decontructor
...
public:
	Shader() {}
	// takes the 'Vertex Shader'(vs) and 'Fragment Shader'(fs)
...
// This class manages input events, window and Opengl render pipeline.
class GLViewer {
public:
	GLViewer();
	~GLViewer();
...
private:
	...
	void render();
	// Everything that needs to be update before rendering mush be done in this method
	void update();
