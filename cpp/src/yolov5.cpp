...
bool Yolov5::initialize_engine(std::string& engine_name) {
	...
	CUDA_CHECK(cudaMalloc(&buffers_[inputIndex], BATCH_SIZE * 3 * INPUT_H * INPUT_W * sizeof (float)));
	CUDA_CHECK(cudaMalloc(&buffers_[outputIndex], BATCH_SIZE * OUTPUT_SIZE * sizeof (float)));
...
void Yolov5::run_inference_and_convert_to_zed() {
